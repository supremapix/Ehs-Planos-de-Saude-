...
const FAQ: React.FC = () => {
  ...
  return (
    <section id="faq" className="py-24 bg-white" ref={sectionRef}>
      <div className={`container mx-auto px-4 max-w-4xl transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        ...
        <div className="space-y-4">
          {FAQS.map((faq, index) => (
            <div 
              key={index}
              className={`faq-item group border rounded-2xl transition-all duration-300 overflow-hidden ${
                openIndex === index 
